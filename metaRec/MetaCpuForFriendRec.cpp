...
#include "Edge.h"
extern int isDebug;
MetaCpuForFriendRec::MetaCpuForFriendRec(Socialnet *socialNet,bool outputNegEqualPos,bool outputFileWrite):outputFileWrite(outputFileWrite),outputNegEqualPos(outputNegEqualPos){
...
void MetaCpuForFriendRec::metaCpu(string testDateFileName,string outputFileName,int toItemType,bool useWindowTime){
    ...
    int route2[4]={ITEMTYPE_USER,ITEMTYPE_LOCATION,ITEMTYPE_USER,ITEMTYPE_USER};//"ULUU";
    int route3[3]={ITEMTYPE_USER,ITEMTYPE_LOCATION,ITEMTYPE_USER};//"ULU";
    int route4[4]={ITEMTYPE_USER,ITEMTYPE_LOCATION,ITEMTYPE_LOCATION,ITEMTYPE_USER};//"ULLU";
    int route5[4]={ITEMTYPE_USER,ITEMTYPE_USER,ITEMTYPE_LOCATION,ITEMTYPE_USER};//"UULU";
    ...
    set<int> uToItemSet;//用户在第一个数据集中到达的loc/user集合
    ...
    while (true)
    {
        ...
        int toItemid=atoi(result[1].data());

        Item * user=socialNet->getItemPtrById(userid,ITEMTYPE_USER);
        Item * toItem=socialNet->getItemPtrById(toItemid,toItemType);
        if(user==NULL||toItem==NULL){//如果用户或者位置不存在
            nullCount++;
            continue;
        }
        ...
        if(userid!=lastuserid){
            ...
            userCount++;
            uToItemSet.clear();//清空用户访问的位置集合

            //处理上一个user，把上一个User中的剩余签到作为负例
             if(lastuserid!=-1){
                negativeCount+=lastPosCount;//添加和正例数量相同的负例
                if(lastPosCount>=5||outputNegEqualPos){
        ...
        int reFlag=addAPositiveCase(user,toItem,toItemType,metaFeature,&uToItemSet,metaPathInCount,postiveStr);
        if (reFlag==-2)
        {
...
int MetaCpuForFriendRec::addAPositiveCase(
    //根据当前记录（user,location/user），以及获得的该用户的特征值集合，尝试添加一个正例
    Item * user,
    Item * toItem,
    int itemType,
    FeatureMap * metaFea,
    set<int> *uToItemSet,//用户在用例生成阶段已经计算过的item
    int *metaPathInCount,
    string & postiveStr////存储将要写入文件的正例字符串，在生成测试集阶段，如果正例不足20条就不写入，也不生成负例。
    ){
        EdgeMap *userEdgeList=user->getToItemE(itemType);
        if(userEdgeList->find(toItem->getId())!=userEdgeList->end()){//如果用户在构建社交网络阶段已经访问过该位置
            return -2;
        }
        if(uToItemSet->find(toItem->getId())!=uToItemSet->end()){//用户在用例生成阶段已经访问过//当出现用户在用例生成阶段第二次访问某一位置时，不再生成正例。
            return -1;
        }
        int flag=0;
        ...
        for(int i=0;i<featureCount;i++){//从六个特征map里面分别找lid
            miter=metaFea[i].find(toItem->getId());
        ...
        if(flag==0) return -3;//不属于任何元路径可以访问得到的位置
            ...
            int toItemId=toItem->getId();
            uToItemSet->insert(toItemId);
            if(outputFileWrite){
                stringstream ss;
                string tmp;
                ss<<user->getId()<<","<<toItemId;
                for(int i=0;i<featureCount;i++)
                    ss<<","<<tmpFeature[i];
                ss<<",1";
                ss>>tmp;
...
int MetaCpuForFriendRec::addNegativeCase(int fromid,
    FeatureMap * metaFea,
    ofstream &outFile,
    int count){
    /*
...
void MetaCpuForFriendRec::printFeatureMap(const FeatureMap *fm) const{
